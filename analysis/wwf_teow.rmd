...
Since, we prepared WDPA polygon data for our analysis, we now load the TEOW global shapefile layer from archived file. If you want to reproduce this you will have to download the file to your local machine. 

```{r loadTeow, warning=FALSE, include=TRUE}

# load TEOW global polygons
teow <- 
  read_sf("/home/rstudio/shared/datalake/mapme.protectedareas/input/teow/Terrestrial-Ecoregions-World.gpkg")
```
...
teow <-
  st_transform(teow, st_crs(br_wdpa_subset))
# apply intersection
teow_wdpa_intersection <- 
  st_intersection(teow, br_wdpa_subset)
```

We then compare the number of polygons in both the original and the resulting WDPA layer to see whether our process split up any  polygon from the intersection.

```{r comparerow, warning=FALSE, include=TRUE}
...
teow_wdpa_intersection <- st_transform(teow_wdpa_intersection, area_proj(br_wdpa_raw))
