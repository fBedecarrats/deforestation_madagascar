...
if (file.exists("data_s3/grille_mada_donnees_raster.rds")) {
  ...
  save(grille_mada, "data_s3/grille_mada_donnees_raster.rds")
}
```
...
```{r Synthèse données satellitaires, fig.height = 9}
if (file.exists("data_s3/grille_mada_summary.rds")) {
  ...
  grille_mada_summary <- grille_mada %>%
    ...
    select(-distance, -layer, -depth, -stat) %>%
...
tm_shape(grille_mada_summary) +
  tm_fill(c("distance_minutes_5k_110mio", "mean_clay_5_15cm",
            "tri_mean", "elevation_mean", "treecover_2000", "var_treecover", 
            "sum_emissions"),
          title = c("Temps de parcours jusquà une localité >5K habitants",
                    "Teneur du sol en argile sur 5-15cm de profondeur",
                    "Terrain accidenté (terrain ruggedeness index)",
                    "Couvert arboré en 2000", 
                    "Variation du couvert arboré de 2000 à 2020",
                    "Emissions en CO2eq"),
          palette = list("Oranges", "YlOrBr", "Greys", "Purples", "Greens", "Reds", "Blues")) +
  tm_layout(main.title = "Données satellitaires agrégées par hexagone",
            main.title.size = taille_titres_cartes,
            legend.outside = TRUE)
