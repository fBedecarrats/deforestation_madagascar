...
dir.create("data_s3/mapme/traveltime")
dir.create("data/tmp")
...
aoi2 <- init_portfolio(x = aoi, 
                       ...
                       add_resources = FALSE,
                      verbose = TRUE)
aoi2 <- get_resources(aoi, resource = "nelson_et_al",
...
for(i in 1:length(aoi3)){
  filename = file.path("data_s3/mapme/traveltime", paste0("hexa_", i, ".rds"))
  if(file.exists(filename)) next
  tmp = aoi3[[i]]
...
aoi3 = calc_indicators(aoi2, indicators = "traveltime", 
                              stats_accessibility = c("mean", "max", "min"), engine = "extract")


# rds_files = list.files("data_s3/mapme/traveltime", pattern = ".rds$", full.names = TRUE)
# # grille_mada = lapply(rds_files, function(f) readRDS(f))
# # grille_mada = st_as_sf(purrr::map_dfr(grille_mada, rbind))
