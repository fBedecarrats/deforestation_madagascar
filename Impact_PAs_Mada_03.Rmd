...
aoi2 <- init_portfolio(x = aoi, 
                       ...
                       outdir = "data_s3/mapme",
                        tmpdir = "tmp",
                       cores = 16, # étendre pour plus d'efficacité 
                      add_resources = FALSE,
                      verbose = TRUE)
...
for(i in 1:length(aoi3)){
  filename = file.path("data_s3/mapme/soilgrids", paste0("hexa_", i, ".rds"))
  if(file.exists(filename)) next
  tmp = aoi3[[i]]
  tmp = calc_indicators(aoi2, 
                        indicators = c("soilproperties"),
                        stats_soil = c("mean"),
                        engine = "extract")
  saveRDS(tmp, filename)
...
rds_files = list.files("data_s3/mapme/soilgrids", pattern = ".rds$", full.names = TRUE)
grille_mada = lapply(rds_files, function(f) readRDS(f))
grille_mada = st_as_sf(purrr::map_dfr(grille_mada, rbind))
